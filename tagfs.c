 ...
 */
 void inotify_read() {
  ...
  void complete_regex(char** const path_tags, int size, char** tags) {
  	regex_t regex[size];
  	char* msgbuf;
  	int i;
  /* Compile regular expressions */
  	for(i=0; i < size; i++){
  		if(regcomp(regex+i, path_tags[i], 0)){
  	...
  	for(i=0; i < size; i++){
  		HASH_ITER(hh, tag_files, current, tmp) {
  			if(!regexec(regex+i, current->name, 0, NULL, 0)){
  				// Match
  				tags[i] = malloc(sizeof(char)*strlen(current->name));
  				strcpy(tags[i], current->name);
  				break;  
  			}
  		}
  	}
